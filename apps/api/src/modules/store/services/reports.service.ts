...
export class StoreReportsService {
  ...
  async getProductSalesReport(query: ReportsQueryDto): Promise<SalesReportByProduct[]> {
    const report = await this.getSalesAnalytics(query);
    return report.byProduct;
  }

  async exportSalesCsv(query: ReportsQueryDto): Promise<Buffer> {
    const { start, end } = resolvePeriodDates(query.period, query.startDate, query.endDate);
